...
The request structure is the same as [server-side file upload API](./server-side-file-upload.md#request-structure-multipart-form-data), except that the client-side application needs to pass `signature`, one-time `token`, and `expire` parameter. These three parameters are explained below.
