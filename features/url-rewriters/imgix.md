...
| <p>aspect ratio</p> | <ul> <li>`ar=2:3` => `ar-2-3`</li><li>`ar=3:1` => `ar=3-1`</li><li>`ar=2:4` => `ar=2-4`</li> </ul> | <ul>Aspect ratio will only work when we  pass either width or height (not both) and fit</ul> |
...
| <p>fit</p> | <ul> <li>`fit=clip` => `c-at_max_enlarge`</li><li>`fit=fill` => `cm-pad_resize`</li><li>`fit=scale` => `c-force`</li> </ul> | <ul>Supported values: <ul><li>clip => c-at_max_enlarge </li><li>crop => no mapping required</li><li>fill => cm-pad_resize</li><li>fillmax => cm-pad_resize </li><li>max => c_at_max </li><li>scale => c-force</li><li>facearea => fo-face</li></ul></ul><ul>For fit=fillmax when transformed dimensions are more than original dimensions then padding will only take place either along h or w not both </ul> |
