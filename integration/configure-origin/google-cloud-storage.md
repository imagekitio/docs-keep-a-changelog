...
3. Give your origin a name. It will appear in the list of origins you have added. For example - **My Google storage bucket**.
4. Enter `https://storage.googleapis.com` in the base URL field.
...
The integration with Google Cloud storage works like [web server](web-server-origin.md) integration. It requires [making all objects in a bucket publicly readable](https://cloud.google.com/storage/docs/access-control/making-data-public#rest-make-bucket-public). It is recommended to use roles/storage.legacyObjectReader instead of roles/storage.objectViewer so that no one can list the objects in your bucket.
{% endhint %}
...
* **Original image through Azure blog storage \(old URL\)** [https://storage.googleapis.com/rest-of-the-path.jpg](https://storage.googleapis.com/rest-of-the-path.jpg)
...
So when you request `https://ik.imagekit.io/your_imagekit_id/rest-of-the-path.jpg`, ImageKit.io internally fetches the file from`https://storage.googleapis.com/rest-of-the-path.jpg`.
